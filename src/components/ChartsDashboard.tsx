...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  // ];
  // Safe ROI buckets with null-check
const roiBuckets = [
  {
    label: '<200',
  ...
  {
    label: '200–500',
    count: tasks.filter(t => typeof t.roi === 'number' && t.roi >= 200 && t.roi <= 500).length,
  },
  {
    label: '>500',
  ...
  {
    label: 'N/A',
    count: tasks.filter(t => t.roi == null || !Number.isFinite(t.roi)).length,
  },
];
